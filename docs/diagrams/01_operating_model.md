...
この図は `docs/00_project_playbook.md` の補助図です。作業の場（ChatGPT / Codex / GitHub）と、運用の主フロー（幹スレッド → 枝スレッド → Codex 作業 → Gitブランチ → プルリク）に加えて、マージ後の資産反映と、幹スレッド・枝スレッドが参照する共有情報源の関係を分けて示します。
